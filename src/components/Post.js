    ...
    return (
            <article className="media" style={{marginBottom:'1rem'}}>
                ...
                <div className="media-content">
                    <p className="title is-size-4-desktop is-size-6-mobile lien">{postTitle}</p>
                    <p className="subtitle is-size-6-desktop is-size-7-mobile">
                        <span className="tag"><b>#{index+1}</b></span> <span className="tag"><b>{formatLargeNumber(postTotalClapCount)} <span aria-label="clap" role="img" >👏</span></b></span>
                    </p>
                    <div className="content is-hidden-mobile">
                            <span style={{'whiteSpace': 'nowrap'}}> Le {new Date(postFirstPublishedAt).toLocaleDateString('fr-FR', options)}</span> 
                            <span> par <a href={`https://medium.com/@${userName}`}>{name}</a></span> 
                    </div>
